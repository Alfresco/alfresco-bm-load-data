...
public class AssignRMRole extends AuthenticatedHttpEventProcessor
{
    ...
    public AssignRMRole(HttpClientProvider httpClientProvider, AuthenticationDetailsProvider authenticationDetailsProvider, String baseUrl,
    ...
    public EventResult processEvent(Event event) throws Exception
    {
        ...
        HttpResponse httpResponse = executeHttpMethodAsAdmin(assignRoleRequest, SimpleHttpRequestCallback.getInstance());
        ...
        if (httpStatus.getStatusCode() != HttpStatus.SC_OK)
        {
            if (httpStatus.getStatusCode() == HttpStatus.SC_CONFLICT)
            {
                ...
                return new EventResult(String.format("Ignoring assign rm role %s, already present in alfresco: ", role), nextEvent);
            }
            else
            {
                String msg = String.format("Assign an rm role :%S to user %s failed, REST-call resulted in status:%d with error %s ", role, username,
        ...
        else
        {
            ...
            return new EventResult(String.format("RM role %s assigned to user %s", role, username), nextEvent);
        }
    }
}
