...
public class PrepareSmartFolders extends AbstractEventProcessor
{
     ...
     * @param password
     *        CMIS password
     * @param smfTemplatePath
     *        path to the smart folder template
     * @param smfRatio
     *        smart folder ratio
     * @param createSmartFolderEvent
     *        event name of event that will create the smart folders
     * @param createdSmartFolderEvent
     *        event name of event to execute when smart folders created
     * @param eventNameSelf
     *        Name of this event processor
     */
    public PrepareSmartFolders(SiteDataService siteDataService,
            boolean smfEnabled, String cmisBindingUrl, String cmisBindingType,
            OperationContext cmisCtx, String username, String password,
            String smfTemplatePath, double smfRatio,
            String createSmartFolderEvent, String createdSmartFolderEvent,
    ...
    protected EventResult processEvent(Event event) throws Exception
    {
        ...
        if (this.smfEnabled)
        {
            ...
            long smartFoldersToCreate = (long) ((double) maxSites
                    * this.smfRatio);
            ...
            Document cmisObject = (Document) cmisSession
                    .getObjectByPath(this.smfTemplatePath);
            Property<?> propId = cmisObject.getProperty("alfcmis:nodeRef");
            String nodeRef = "N" + propId.getValueAsString();

            // get or create event data
            DBObject eventData = (event.getData() instanceof BasicDBObject)
                    ? (BasicDBObject) event.getData()
                    : new BasicDBObject();
            ...
            List<SiteData> sites = this.siteDataService.getSites(null, null,
                    (int) smartFoldersCreatedCount, max);
            ...
            while (smartFoldersToCreate > 0 && preparedCount < max)
            {
                ...
                DBObject newEventData = new BasicDBObject()
                        .append(SiteData.FIELD_SITE_ID, sd.getSiteId())
                        .append(SITE_COUNT, preparedCount)
                        .append(NODE_REF, nodeRef);
                Event siteEvent = new Event(this.createSmartFolderEvent,
                        newEventData);
            ...
            if (smartFoldersToCreate > 0)
            {
                ...
                msg = "Prepared "
                        + preparedCount
                        + " sites. Rescheduled self to reach a total of "
                        + (smartFoldersCreatedCount + smartFoldersToCreate)
                        + " smart root folders";
                ...
                if (preparedCount > 0)
                {
                    msg = "Prepared " + preparedCount
                            + " sites to receive a smart root folder.";
                }
        }
